import 'package:flutter/material.dart';

class ColorManager{
  static const Color primaryColor =  Color(0xFFD68469);
  static const Color scaffoldBackgroundColor =  Color(0xFFB1BF97);

  static const Color hintTextColor =  Color(0xFF717171);

  static  const Color blackColor =  Color(0xFF000000);

  static const Color whiteColor =  Color(0xFFFFFFFF);

  static const Color grayColor =  Color(0xFFf4f4f4);

  static const Color errorColor =  Color(0xFFED1C24);

  static const Color successColor =  Color(0xFF62B587);


  static Color unselectedItemColor =  Colors.grey.shade300;

  static Color shadowColor =  Color(0xffE0E0E0);

  static Color rateStarColor =  Color(0xfffdf301);



}